...
static const float noiseSize = 1024; // scale for the noise used for snow coverage
...
float4 SnowCoverage( VSOUT IN ) : COLOR0
{
    ...
    if (uv.x > 1 || uv.y > 1) return float4(1, 1, 1, 1);

    float3 world = toWorld(uv);
    float depth = readDepth(uv);

    // Early out for sky pixels
    if (depth > TESR_OrthoData.x) return float4(1, 1, 1, 1);
...
ortho += GetOrtho(worldPos + float4(0, 0.3, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(-0.5, -0.5, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(-0.5, 0.5, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0.5, -0.5, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0.5, 0.5, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(-0.8, 0, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0.8, 0, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0, -0.8, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0, 0.8, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(-0.2, 0, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0.2, 0, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0, -0.2, 0, 0) * orthoRadius);
ortho += GetOrtho(worldPos + float4(0, 0.2, 0, 0) * orthoRadius);
ortho /= 25;
...
float4 Snow(VSOUT IN): COLOR0 {
    ...
    float waterTreshold = (depth / farZ) * 100;
    ...
    //float3 sparkles = pows(shades(eyeDirection, normalize(expand(tex2D(TESR_BlueNoiseSampler, worldPos.xy / 200).rgb))), 100) * 0.2 * shadow.r;
	...
	float timeToAppear = 0.5; // Time in game units for snow to fully appear
	float currentTime = TESR_SnowAccumulationParams.w;

    // Calculate the distance from the current pixel to the water surface and blend
    float distanceToWater = abs(worldPos.z - TESR_WaterSettings.x +10.0 );
    float blendBandWidth = 10.0; // Adjust this value as needed
    float blendFactor = saturate((distanceToWater - blendBandWidth) / blendBandWidth);
    snowColor.rgb = lerp(color.rgb, snowColor.rgb, blendFactor);

    // Optionally adjust alpha based on the blending factor to create a transparent blend band
    snowColor.a = lerp(color.a, 1.0, blendFactor);

    // create a noisy pattern of accumulation over time
    float3 worldPosNoOffset = worldPos.xyz / (noiseSize * 0.5); // Remove camera offset
	
	float coverage = saturate((currentTime / timeToAppear) * pows(noise(worldPosNoOffset) * noise(worldPosNoOffset * 3) * noise(worldPosNoOffset), 2 - 2 * TESR_SnowAccumulationParams.w));
	
    coverage = saturate(smoothstep(0, 0.4, coverage));

    // Apply additional blur to non-full snow-covered parts
    if (coverage > 0.01 && coverage < 0.99) // Apply blur only when coverage is partial
    {
        float blurFactor = 0.9 + coverage; // Calculate blur factor based on the non-full snow coverage
        float blur = tex2D(TESR_BlueNoiseSampler, worldPos.xy / 512).r; // Adjust blur texture as needed
        blur += blurFactor; // Multiply blur by the blur factor to affect only non-full snow-covered parts
        coverage *= blur; // Add the blurred result to the coverage
    }
...
// Add blur to ortho
    // Apply blur only when ortho is between 0.1 and 1	
    if (ortho > 0.01 && ortho < 0.8) {
        // Sample blue noise texture instead of basic noise
        float blurOrtho = tex2D(TESR_BlueNoiseSampler, worldPos.xy / 256).r; // Adjust blur texture as needed

        float blurStrength = 0.1 + ortho; // Adjust as needed
        blurOrtho += blurStrength; // Ensure blurOrtho stays within valid range [0, 1]
        ortho *= blurOrtho;
    }
    if (ortho > 0.3 && ortho < 1) {
        // Sample blue noise texture instead of basic noise
        float blurOrtho = tex2D(TESR_BlueNoiseSampler, worldPos.xy / 4096).r; // Adjust blur texture as needed
    ...
    ortho = saturate(ortho); // Ensure ortho value stays within the valid range

    float vertical = smoothstep(0.1, 1, shade(float3(0, 0, 1), norm));

       
	snowColor.rgb = lerp(color.rgb, snowColor.rgb, coverage * min(vertical, ortho));
    snowColor.a = 1;

    snowColor.rgb = delinearize(snowColor);
